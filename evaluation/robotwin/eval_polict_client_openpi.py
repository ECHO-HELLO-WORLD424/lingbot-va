...
robowin_root = Path(__file__).resolve().parents[2] / "robotwin"
...
def visualize_action_step(action_history, step_idx, window=50):
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
        2, 2, figsize=(14, 8), dpi=100, sharex=True
    )
...
def format_obs(observation, prompt):
    return {
        "observation.images.cam_high": observation["observation"]["head_camera"][
            "rgb"
        ],  # H,W,3
        "observation.images.cam_left_wrist": observation["observation"]["left_camera"][
            "rgb"
        ],
        "observation.images.cam_right_wrist": observation["observation"][
            "right_camera"
        ]["rgb"],
...
def eval_policy(
    ...
    while succ_seed < test_num:
        ...
        ret = model.infer(
            dict(reset=True, prompt=prompt, save_visualization=save_visualization)
        )
        ...
        vis_dir = (
            Path(args["save_root"]) / f"stseed-{st_seed}" / "visualization" / task_name
        )
...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--port", type=int, default=8000, help="remote policy socket port."
    )
